...
export default class DownloadHandler {
  ...
  constructor() {
    ...
    listen('download_error', ({ payload }) => {
      ...
      const index = this.downloads.findIndex(
        (download) => download.path === errorData.path
      );
    ...
    listen('extract_start', ({ payload }) => {
      ...
      const index = this.downloads.findIndex(
        (download) => download.path === payload
      );
    ...
    listen('extract_end', ({ payload }) => {
      ...
      const index = this.downloads.findIndex(
        (download) => download.path === obj.file || obj.new_folder
      );
      this.downloads[index].status = 'finished';
    });
  }
  ...
  getTotalAverage() {
    ...
    const progress = files.reduce(
      (acc, d) =>
        d.progress !== 0 ? acc + d.progress : acc + d.total_downloaded,
      0
    );
    let speedStr = '0 B/s';
    ...
    return {
      ...
      extracting: this.downloads.filter((d) => d.status === 'extracting')
        .length,
      totalSize: total,
      speed: speedStr,
    };
