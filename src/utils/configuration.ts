...
export async function setConfigOption<K extends keyof Configuration>(
  key: K,
...
export async function getConfigOption<K extends keyof Configuration>(
  key: K
): Promise<Configuration[K]> {
  const config = await getConfig();
  const defaults = defaultConfig;

  return config[key] === null || config[key] === undefined
    ? defaults[key]
    : config[key];
}
...
async function readConfigFile() {
  ...
  if (!innerDirs.find((fileOrDir) => fileOrDir?.name === 'grasscutter')) {
    // Create dir
    await fs
      .createDir(local + 'cultivation/grasscutter')
  ...
  if (
    !dataFiles.find((fileOrDir) => fileOrDir?.name === 'configuration.json')
  ) {
    // Create config file
